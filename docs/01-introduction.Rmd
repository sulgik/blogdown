...
이 장에서, we show how to create a simple website from scratch using **blogdown**. The website will contain a home page, an "About" page, one R Markdown post, and two plain Markdown posts. You will learn the basic concepts for creating websites with **blogdown**. 
...
LiveReload 를 *실습*해보자. In your files pane, navigate inside the `content/` folder. This folder is where all your site's content lives. Find and open the file `content/post/2020-12-01-r-rmarkdown/index.Rmd`. This is a sample R Markdown post that is added by **blogdown** by default when you set up a new site. 
...
### 의심되면 `blogdown::check_site()` 을 실행하라
